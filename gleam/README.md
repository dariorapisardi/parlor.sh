...
The parlor server, in Gleam on the BEAM. It began as a port of a zero-dependency Node server
(`server.mjs`, in git history until 2026-09-23) and keeps its HTTP contract, environment variables
and data directory unchanged; it has served parlor.sh since 2026-09-23 (`deploy/parlor-gleam.service`,
`deploy/DEPLOY.md`). Why the BEAM: availability. A room is a process, and later rooms can live on
...
| `src/parlor/store.gleam` | the files, in the original Node server's formats |
...
```
tests/conformance/conformance.py --cmd "sh gleam/build/erlang-shipment/entrypoint.sh run" --then "sh gleam/build/erlang-shipment/entrypoint.sh run"
```

The contract, the limits, and rooms that outlive a restart of the server. To check that a change
keeps an older build's data working, give `--then` the older build's command.
